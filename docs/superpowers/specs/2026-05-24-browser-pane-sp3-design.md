...
implemented; P3 history/bookmarks is complete (core stores, socket verbs, CLI
mirrors, plus GTK visit recording and address-bar completion); P4 import is still
backlog. WebKit/GTK browser code
...
| P3 History+bookmarks | per-profile history/bookmark stores, socket verbs, CLI mirrors, GTK visit recording, and address-bar completion are implemented | no | `rusqlite` |
...
3. **P3** history+bookmarks: `browser_history.rs`, socket verbs, CLI mirrors,
   GTK visit recording, and address-bar completion — all implemented.
