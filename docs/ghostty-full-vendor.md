...
through `ghostty_gtk_surface_read_text_limited_with_total_lines` into the session on child exit,
...
`ghostty_gtk_surface_read_text_limited_with_total_lines` export. The packagers
install the verified library into `usr/lib`, and the binary loads it through its
RUNPATH (`$ORIGIN/../lib`). The install step is required for release packages:
