...
exists.
Provider hook permission modes remain display-only metadata for status and
agent-session inspection. ForkTTY never copies hook-reported permission modes
into resume argv, so socket or hook metadata cannot enable dangerous provider
bypass flags.
