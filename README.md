...
[![Release](https://img.shields.io/github/v/release/Lucenx9/forktty?include_prereleases)](https://github.com/Lucenx9/forktty/releases/tag/v0.2.0-alpha.13)
...
[Download v0.2.0-alpha.13 AppImage](https://github.com/Lucenx9/forktty/releases/download/v0.2.0-alpha.13/forktty-0.2.0-alpha.13-x86_64.AppImage)

</div>

> **Status**: Early alpha (v0.2.0-alpha.13). ForkTTY is Linux-only and the GTK/Ghostty runtime is now the primary implementation. The AppImage is the primary Linux download for this alpha; the Debian package remains available for Debian/Ubuntu users.
...
[v0.2.0-alpha.13 release](https://github.com/Lucenx9/forktty/releases/tag/v0.2.0-alpha.13).
Each release ships:

- `forktty-0.2.0-alpha.13-x86_64.AppImage` — recommended portable Linux package.
- `forktty-0.2.0-alpha.13-x86_64.AppImage.zsync` — AppImage delta-update metadata for external AppImage managers.
- `forktty_0.2.0.alpha.13_amd64.deb` — Debian/Ubuntu package.
...
```bash
chmod +x forktty-0.2.0-alpha.13-x86_64.AppImage
./forktty-0.2.0-alpha.13-x86_64.AppImage
```
...
```bash
GSK_RENDERER=ngl ./forktty-0.2.0-alpha.13-x86_64.AppImage
```

### Debian / Ubuntu (.deb)

```bash
sudo apt install ./forktty_0.2.0.alpha.13_amd64.deb
# or, if apt cannot read the file path directly:
sudo dpkg -i forktty_0.2.0.alpha.13_amd64.deb
sudo apt -f install
```
...
- OSC 9 and basic OSC 99 terminal notifications are parsed from the Ghostty-owned PTY stream and rate-limited per surface; advanced OSC 99 features such as base64 payloads, update/close controls, activation reports, buttons, and full chunk aggregation remain partial.
