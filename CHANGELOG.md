...
- Kitty image snapshots now copy only the visible source rectangle for each
  placement and use fallible render-buffer allocation, preventing malicious
  terminal output from forcing full-image per-placement copies on every redraw.
