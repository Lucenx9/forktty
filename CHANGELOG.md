...
- Shell trampoline detection now recognizes PowerShell's command grammar, so `pwsh -Command ...` and `pwsh -EncodedCommand ...` notification commands are rejected like the previously caught `pwsh -c` instead of bypassing the shell-command guard.
