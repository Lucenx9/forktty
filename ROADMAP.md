...
- [x] Ghostty config appearance import for font, colors, `theme`, `config-file`, named colors, ANSI palette entries, `scrollback-limit`, cursor style/blink defaults, selection clear-on-typing/copy, selection word chars, copy-on-select, right-click action, scroll-to-bottom, mouse reporting/shift-capture/hide-while-typing including XTSHIFTESCAPE overrides, cursor/faint opacity, mouse scroll multiplier, cell size adjustments, and inactive split dimming.
