...
- [x] Ghostty config appearance import for font, colors, `theme`, `config-file`, named colors, ANSI palette entries, `scrollback-limit`, cursor style/blink defaults, selection clear-on-typing, right-click action, cursor/faint opacity, mouse scroll multiplier, cell size adjustments, and inactive split dimming.
