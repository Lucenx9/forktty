...
| Renderer parity | Default terminal panes use the embedded Ghostty GTK widget when `ghostty-gtk-embed.so` is available; a pane that cannot load or spawn embedded falls back to the classic GTK/Pango/Cairo renderer. The full upstream Ghostty source is pinned at `vendor/ghostty` for the cmux-style renderer/widget integration. Upstream's current public C embedding API is macOS/iOS-only for surfaces, while the Linux `GhosttySurface` GTK widget is internal to Ghostty's GTK app runtime. ForkTTY's Ghostty fork now carries a GTK widget embedding ABI with cwd, socket text-input, and visible/full text-read hooks. Embedded panes also mirror Ghostty surface title, child-exit readiness/status (with the real exit code via `ghostty_gtk_surface_exit_code`, falling back to a neutral "Closed" on older libraries), abnormal-exit notifications, and close-request teardown into the model via GObject signals. Embedded panes reach copy/paste/select-all/find parity through `ghostty_gtk_surface_perform_action`, which performs a Ghostty keybinding action by name on the focused surface (copy/paste/select-all route the `Ctrl+Shift+C/V/A` accelerators and command-palette actions; find opens Ghostty's native search overlay via `start_search`); mouse selection works natively inside the surface, and libraries lacking the symbol degrade to a logged no-op. Embedded panes also expose their child PID via `ghostty_gtk_surface_child_pid` (fed by a `pid_available` surface mailbox message that hands the IO-thread-owned pid to the GTK main thread race-free), so listening-port discovery and the socket `surfaces` PID field reach parity; ForkTTY polls the getter briefly after spawn, and libraries lacking the symbol skip the poll. The deb and AppImage packagers require `ghostty-gtk-embed.so` in `usr/lib`, so installed builds load it via the binary RUNPATH (`$ORIGIN/../lib`) without `FORKTTY_GHOSTTY_GTK_LIB`; release CI builds the library before packaging and fails if it is absent. When `appearance.persistent_scrollback_lines` is greater than zero, embedded panes also snapshot their scrollback tail into the session (on child exit, on programmatic close/restart, and via a throttled poll) like classic panes; restoring that tail into a respawned surface is wired through `ghostty_gtk_surface_restore_scrollback`, which feeds Ghostty's VT stream rather than the child PTY. Set `appearance.embedded_ghostty = false` or `FORKTTY_GHOSTTY_GTK_PANES=0` to opt out of embedded panes; force-on values are `1`, `true`, `yes`, and `on`. `forktty doctor` warns about a missing embedding library when embedded panes are enabled, and stays quiet when the user opts out. Rows 4/6/7/8 in `docs/ghostty-embedded-parity.md` remain deferred manual validation items, not default-switch blockers. |
