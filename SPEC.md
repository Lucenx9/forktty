...
| Renderer parity | Partial by design: ForkTTY currently paints Ghostty cells with a GTK/Pango/Cairo renderer. Kitty PNG images are drawn from libghostty-vt placements. The full upstream Ghostty source is pinned at `vendor/ghostty` for the cmux-style renderer/widget integration spike. Upstream's current public C embedding API is macOS/iOS-only for surfaces, while the Linux `GhosttySurface` GTK widget is internal to Ghostty's GTK app runtime, so release runtime still waits on a minimal Ghostty GTK embedding API or an explicit small Ghostty fork patch that fits ForkTTY panes, splits, socket automation, and session restore. |
