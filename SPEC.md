...
| Terminal appearance | Supported for terminal font family/style fallbacks, font size, foreground/background, cursor colors, DECSCUSR-backed cursor style defaults, selection, bold, faint, ANSI palette, named colors, short/full hex colors, `theme`, cell metric adjustment, scrollback limit, image storage limit, mouse scroll multiplier, and inactive split dimming. |
