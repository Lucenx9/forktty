...
Hook-reported permission-mode status entries are display-only metadata. They may appear in status and agent-session metadata, but they are never copied into provider argv for `agent.health`, `agent.resume`, or restore-time auto-resume; a socket or hook status update cannot enable dangerous provider bypass flags.
