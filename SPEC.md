...
The GTK workspace sidebar is a collapsed libadwaita overlay by default, so
showing or hiding it does not resize the terminal layout. Its header pin
control sets `appearance.sidebar_pinned = true` and presents the sidebar beside
the terminal instead. `appearance.sidebar_position` chooses the left or right
edge, while `appearance.sidebar_visible` remains the independent persisted
startup preference used by Settings and the Ctrl+B/F9 action.
