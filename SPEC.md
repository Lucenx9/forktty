...
| Renderer parity | Partial by design: ForkTTY currently paints Ghostty cells with a GTK/Pango/Cairo renderer. Kitty PNG images are drawn from libghostty-vt placements. The full upstream Ghostty source is pinned at `vendor/ghostty` for the cmux-style renderer/widget integration spike. Upstream's current public C embedding API is macOS/iOS-only for surfaces, while the Linux `GhosttySurface` GTK widget is internal to Ghostty's GTK app runtime. ForkTTY's Ghostty fork now carries an experimental GTK widget embedding ABI with cwd and socket text-input hooks, but release runtime still uses the existing renderer until read-text/capture, title/status, PID/exit lifecycle, packaging, and parity validation are complete. |
