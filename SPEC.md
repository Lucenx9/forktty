...
targets when a tool omits an explicit target. `team_upsert` prefers
`FORKTTY_SURFACE_ID` as `leader_surface_id`; it falls back to
`FORKTTY_WORKSPACE_ID` only when no leader surface is available.
