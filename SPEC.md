...
| Renderer parity | Partial by design: ForkTTY currently paints Ghostty cells with a GTK/Pango/Cairo renderer. Kitty PNG images are drawn from libghostty-vt placements. The full upstream Ghostty source is pinned at `vendor/ghostty` for the cmux-style renderer/widget integration spike. Upstream's current public C embedding API is macOS/iOS-only for surfaces, while the Linux `GhosttySurface` GTK widget is internal to Ghostty's GTK app runtime. ForkTTY's Ghostty fork now carries an experimental GTK widget embedding ABI with cwd, socket text-input, and visible/full text-read hooks. Experimental embedded panes also mirror Ghostty surface title, child-exit readiness/status (with the real exit code via `ghostty_gtk_surface_exit_code`, falling back to a neutral "Closed" on older libraries), abnormal-exit notifications, and close-request teardown into the model via GObject signals. Embedded panes reach copy/paste/select-all/find parity through `ghostty_gtk_surface_perform_action`, which performs a Ghostty keybinding action by name on the focused surface (copy/paste/select-all route the `Ctrl+Shift+C/V/A` accelerators and command-palette actions; find opens Ghostty's native search overlay via `start_search`); mouse selection works natively inside the surface, and libraries lacking the symbol degrade to a logged no-op. The child PID is not yet exposed by the embedding ABI, so listening-port discovery is unavailable for embedded panes. The deb and AppImage packagers install `ghostty-gtk-embed.so` into `usr/lib` when the probe script has built it, so installed builds load it via the binary RUNPATH (`$ORIGIN/../lib`) without `FORKTTY_GHOSTTY_GTK_LIB`; the step is optional and release CI does not build the library by default yet, so stable artifacts still omit it. `forktty doctor` warns about a missing embedding library only when embedded panes are opted in via `FORKTTY_GHOSTTY_GTK_PANES`. Release runtime still uses the existing renderer until PID lifecycle parity, default-on packaging, and parity validation are complete. |
