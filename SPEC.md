...
| Renderer parity | Partial by design: ForkTTY currently paints Ghostty cells with a GTK/Pango/Cairo renderer. Kitty PNG images are drawn from libghostty-vt placements. The full upstream Ghostty source is pinned at `vendor/ghostty` for the cmux-style renderer/widget integration spike. Upstream's current public C embedding API is macOS/iOS-only for surfaces, while the Linux `GhosttySurface` GTK widget is internal to Ghostty's GTK app runtime. ForkTTY's Ghostty fork now carries an experimental GTK widget embedding ABI with cwd, socket text-input, and visible/full text-read hooks. Experimental embedded panes also mirror Ghostty surface title, child-exit readiness/status (with the real exit code via `ghostty_gtk_surface_exit_code`, falling back to a neutral "Closed" on older libraries), abnormal-exit notifications, and close-request teardown into the model via GObject signals. Embedded panes reach copy/paste/select-all/find parity through `ghostty_gtk_surface_perform_action`, which performs a Ghostty keybinding action by name on the focused surface (copy/paste/select-all route the `Ctrl+Shift+C/V/A` accelerators and command-palette actions; find opens Ghostty's native search overlay via `start_search`); mouse selection works natively inside the surface, and libraries lacking the symbol degrade to a logged no-op. Embedded panes also expose their child PID via `ghostty_gtk_surface_child_pid` (fed by a `pid_available` surface mailbox message that hands the IO-thread-owned pid to the GTK main thread race-free), so listening-port discovery and the socket `surfaces` PID field reach parity; ForkTTY polls the getter briefly after spawn, and libraries lacking the symbol skip the poll. The deb and AppImage packagers install `ghostty-gtk-embed.so` into `usr/lib` when the probe script has built it, so installed builds load it via the binary RUNPATH (`$ORIGIN/../lib`) without `FORKTTY_GHOSTTY_GTK_LIB`; release CI now builds the library best-effort before packaging, so stable artifacts ship it when the build succeeds and fall back to the classic renderer only if it fails. When `appearance.persistent_scrollback_lines` is greater than zero, embedded panes also snapshot their scrollback tail into the session (on child exit, on programmatic close/restart, and via a throttled poll) like classic panes; restoring that tail into a respawned surface is wired through an optional `ghostty_gtk_surface_restore_scrollback` ABI that feeds Ghostty's VT stream rather than the child PTY, and degrades to a no-op until the embedding library exports the symbol. Embedded panes are opt-in via the `appearance.embedded_ghostty` config option (off by default) or the `FORKTTY_GHOSTTY_GTK_PANES` environment variable; either opt-in enables them, and a pane that fails to spawn embedded falls back to the classic renderer. `forktty doctor` warns about a missing embedding library only when one of those opt-ins is set. Release runtime still uses the existing renderer until default-on packaging and parity validation are complete. |
