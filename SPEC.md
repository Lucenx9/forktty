...
`team.worker.shutdown` with `close_surface: true` is immediate disposable-pane cleanup after the shutdown text is delivered/submitted; it is not proof that the worker processed a graceful shutdown request. Use it only for launch-owned worker panes that can be discarded.
