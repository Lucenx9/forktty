...
| Renderer parity | Partial by design: ForkTTY currently paints Ghostty cells with a GTK/Pango/Cairo renderer. Kitty PNG images are drawn from libghostty-vt placements. The full upstream Ghostty source is pinned at `vendor/ghostty` for the cmux-style renderer/widget integration spike, but release runtime still waits on an upstream Ghostty embeddable renderer/widget API, or an explicit small Ghostty fork patch, that fits ForkTTY panes, splits, socket automation, and session restore. |
