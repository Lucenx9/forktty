...
fn main() {
    ...
    // FORKTTY PATCH: zig's default optimize mode is Debug, which made every
    // ForkTTY build (including releases) ship a libghostty whose VT parsing
    // ran at ~65 KB/s. ReleaseSafe keeps runtime safety checks on untrusted
    // VT input while restoring optimized codegen.
    let optimize =
        env::var("LIBGHOSTTY_VT_SYS_OPTIMIZE").unwrap_or_else(|_| "ReleaseSafe".to_string());
