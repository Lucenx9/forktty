...
fn build_vendored(link_mode: LinkMode) {
    ...
    if target != host {
        ...
        // FORKTTY PATCH: native zig builds default to the build host's full
        // CPU feature set, baking host-specific ISA extensions into binaries
        // that get distributed (zig's compiler_rt memset, statically linked
        // into the final executable, used AVX-512 when the CI runner had it
        // and crashed every non-AVX-512 machine with SIGILL).
