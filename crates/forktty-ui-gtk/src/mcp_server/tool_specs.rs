        ...
        ToolSpec {
            ...
            input_schema: object_schema(
                ...
                json!({
                    ...
                    "harness_signals": {
                        "type": "object",
                        "description": format!("Optional per-harness routing signals keyed by harness id. readiness=verified_ready requires readiness_reason and promotes a launchable harness only after concrete worker, hook, or user evidence. Values may also include cooldown, cooldown_kind (quota|auth|crash|timeout, requires cooldown=true), cooldown_reason, locked_out, and lockout_reason; reason fields are capped at {} UTF-8 bytes and reject control characters. Cooldown is soft; locked_out excludes assignments. Explicit signals replace inferred cooldown for that harness.", protocol_limits::SOCKET_TASK_STRATEGY_REASON_MAX_BYTES),
                        "additionalProperties": true
                    },
                    ...
                    "repo_dirty": boolean_prop("Whether the repository has uncommitted changes; omitted lets ForkTTY infer from cwd or selected surface/workspace cwd."),
                    ...
                    "user_visible": boolean_prop("Optional override for likely user-visible changes; omitted lets ForkTTY infer from mutating task_kind hints and high-confidence goal text."),
                }),
            ),
        },
        ToolSpec {
            ...
            input_schema: object_schema(
                ...
                json!({
                    ...
                    "approval_id": string_prop("Approved request-bound Feed approval id; can satisfy remaining approvals covered by that request."),
                    ...
                    "cwd": string_prop("Absolute repo cwd inside an open ForkTTY repo context; used for worker launches and prompts when no worktree_name is used."),
