...
Read-only first: use workspace_list, surface_list, topology_tree, surface_read_text, surface_capture_tail, agent_list, agent_health, agent_reclaim_plan, team_list, team_get, team_inbox, team_summary, team_events, and status_summary to inspect state before changing focus, sending text, resuming agents, changing team state, or changing worktrees.
...
Use mutating tools only for visible coordination: surface_focus and surface_send_text operate on panes; agent_resume resumes a persisted session only after agent_health says ready; team_upsert, team_worker_upsert, team_worker_heartbeat, team_task_upsert, team_message_send, and team_message_ack update ForkTTY's team control-plane state; team_worker_launch opens a provider worker tab, while team_message_dispatch, team_worker_nudge, and team_worker_shutdown send text to an attached worker pane; worktree_create, worktree_attach, worktree_remove, and worktree_merge manage parallel git work; status_set and notification_create publish progress or ask for user attention.
...
pub(crate) fn session_context_lines() -> [&'static str; 4] {
    [
        "Use ForkTTY tools when the task involves panes, workspaces, agent sessions, team orchestration state, worktrees, status, terminal read/capture, or sending text to another surface.",
        "For ordinary edits in the current repo, work normally; do not call ForkTTY tools just to edit files.",
        "Read-only first: workspace_list, surface_list, topology_tree, surface_read_text, surface_capture_tail, agent_list, agent_health, agent_reclaim_plan, team_list/get/inbox/summary/events, and status_summary inspect state before any mutating ForkTTY action.",
        "Mutating coordination tools: surface_focus, surface_send_text, agent_resume, team_upsert/worker/task/message tools, worktree_create/attach/remove/merge, status_set, and notification_create.",
    ]
}
