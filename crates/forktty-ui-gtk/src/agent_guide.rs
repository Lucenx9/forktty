...
Read-only first: use workspace_list, surface_list, agent_list, agent_health, agent_reclaim_plan, and status_summary to inspect state before changing focus, sending text, resuming agents, or changing worktrees.
...
pub(crate) fn session_context_lines() -> [&'static str; 4] {
    [
        "Use ForkTTY tools when the task involves panes, workspaces, agent sessions, worktrees, status, or sending text to another surface.",
        "For ordinary edits in the current repo, work normally; do not call ForkTTY tools just to edit files.",
        "Read-only first: workspace_list, surface_list, agent_list, agent_health, agent_reclaim_plan, and status_summary inspect state before any mutating ForkTTY action.",
        "Mutating coordination tools: surface_focus, surface_send_text, agent_resume, worktree_create/attach/remove/merge, status_set, and notification_create.",
    ]
}
