...
Use mutating tools only for visible coordination: surface_focus and surface_send_text operate on panes; agent_hibernate and agent_reclaim close only idle resumable sessions after agent_reclaim_plan/agent_health, agent_resume resumes a persisted session only after agent_health says ready; workflow_upsert, workflow_plan_set, and workflow_evidence_add preserve goal/plan/evidence memory; team_upsert, team_finish, team_worker_upsert, team_worker_heartbeat, team_task_upsert, team_message_send, and team_message_ack update ForkTTY's team control-plane state; team_finish should dry-run before finalizing non-trivial work, use force only after checking team_summary and team_worker_health, and only close launch-owned disposable worker panes; team_worker_launch opens a provider worker tab, while team_message_dispatch, team_worker_nudge, and team_worker_shutdown send text to an attached worker pane; dispatch/shutdown submit appends a carriage-return Enter to the same terminal input. worktree_create, worktree_attach, worktree_remove, and worktree_merge manage parallel git work; status_set and notification_create publish progress or ask for user attention.
