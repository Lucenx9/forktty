...
Use mutating tools only for visible coordination: surface_focus and surface_send_text operate on panes; agent_hibernate and agent_reclaim close only idle resumable sessions after agent_reclaim_plan/agent_health, agent_resume resumes a persisted session only after agent_health says ready; workflow_upsert, workflow_plan_set, and workflow_evidence_add preserve goal/plan/evidence memory; team_upsert, team_finish, team_worker_upsert, team_worker_heartbeat, team_task_upsert, team_message_send, and team_message_ack update ForkTTY's team control-plane state; team_finish should dry-run before finalizing non-trivial work, use force only after checking team_summary and team_worker_health, and only close launch-owned disposable worker panes; team_worker_launch opens a provider worker tab, while team_message_dispatch, team_worker_nudge, and team_worker_shutdown send text to an attached worker pane; dispatch/shutdown submit uses provider-aware terminal input, including a short settle before Claude Enter. worktree_create, worktree_attach, worktree_remove, and worktree_merge manage parallel git work; status_set and notification_create publish progress or ask for user attention.
...
pub(crate) fn session_context_lines() -> [&'static str; 4] {
    [
        "Use ForkTTY tools when the task involves panes, workspaces, SSH remote inventory, agent sessions, workflow memory, team orchestration state, worktrees, status, terminal read/capture, or sending text to another surface.",
        "For ordinary edits in the current repo, work normally; do not call ForkTTY tools just to edit files.",
        "Read-only first: context_snapshot gives a compact situational snapshot with workflow_summaries/team_summaries by default; workspace_list, surface_list, topology_tree, remote_list/status, surface_read_text, surface_capture_tail, agent_list, agent_health, agent_reclaim_plan, status_summary, workflow_list/get/replay, team_list/get/inbox/summary/worker_health/events inspect targeted state before any mutating ForkTTY action.",
        "Mutating coordination tools: surface_focus, surface_send_text, agent_hibernate/reclaim/resume, workflow_upsert/plan_set/evidence_add, team_upsert/finish/worker/task/message tools, worktree_create/attach/remove/merge, status_set, and notification_create.",
    ]
}
