...
pub(in crate::socket_cli) fn build_hook_shell_command(
    ...
    let extract_env = if launcher_uses_appimage_runtime(launcher) {
        "APPIMAGE_EXTRACT_AND_RUN=1 "
    } else {
        ""
    };
    format!(
        "[ \"${{{}:-}}\" != \"1\" ] && {extract_env}{} hooks {} {} || echo '{}'",
        spec.disabled_env,
...
pub(in crate::socket_cli) fn build_opencode_plugin(launcher: &Path) -> CliResult<String> {
    let appimage_extract_and_run = launcher_uses_appimage_runtime(launcher);
...
const APPIMAGE_EXTRACT_AND_RUN = {appimage_extract_and_run};
...
function runForkTTY(hookEvent, body) {{
  ...
  const env = APPIMAGE_EXTRACT_AND_RUN
    ? {{ ...process.env, {appimage_env}: {appimage_env_value} }}
    : process.env;
...
"#,
        ...
        appimage_env = serde_json::to_string(APPIMAGE_EXTRACT_AND_RUN_ENV)?,
        appimage_env_value = serde_json::to_string(APPIMAGE_EXTRACT_AND_RUN_VALUE)?,
