...
Usage:
  ...
  forktty task-plan <goal> [workspace selectors] [--surface-id <id>] [--cwd <repo>] [--strategy <strategy>] [--task-kind <kind>] [--profile balanced|fast|conservative|parallel|review_heavy] [--last-known-good-json <json>] [--harness-signals-json <json>] [--repo-dirty] [--parallel] [--review] [--user-visible] [--json]
      Ask ForkTTY to choose a read-only strategy for a task before selecting team, loop, worktree, or harnesses. The goal is capped at 4096 UTF-8 bytes and rejects terminal control characters other than newline or tab before it is copied into workflow/team state or worker prompts. If --profile is omitted, ForkTTY infers a router profile when the goal clearly asks for speed, caution, parallel work, or review. ForkTTY can infer last-known-good stickiness from completed task-strategy workflows; --last-known-good-json may override or enrich it. --harness-signals-json may pass per-harness cooldown/lockout signals for scripts; for harnesses it does not name, ForkTTY infers an advisory soft cooldown from recent failed task-strategy workflows. If --repo-dirty or --user-visible is omitted, ForkTTY infers dirty state from --cwd inside an open ForkTTY workspace/surface repo or the selected surface/workspace cwd, and edit intent from --task-kind plus high-confidence goal wording when possible.
