...
Usage:
  ...
  forktty task-plan <goal> [workspace selectors] [--surface-id <id>] [--cwd <repo>] [--strategy <strategy>] [--profile balanced|fast|conservative|parallel|review_heavy] [--last-known-good-json <json>] [--harness-signals-json <json>] [--repo-dirty] [--parallel] [--review] [--user-visible] [--json]
      Ask ForkTTY to choose a read-only strategy for a task before selecting team, loop, worktree, or harnesses. If --profile is omitted, ForkTTY infers a router profile when the goal clearly asks for speed, caution, parallel work, or review. ForkTTY can infer last-known-good stickiness from completed task-strategy workflows; --last-known-good-json may override or enrich it. --harness-signals-json may pass per-harness cooldown/lockout signals for scripts. If --repo-dirty or --user-visible is omitted, ForkTTY infers dirty state from --cwd or the selected surface/workspace cwd and edit intent from the goal when possible.
  forktty task-apply --run-id <id> --plan-json <json> [--workspace-id <id>|--workspace-name <name>|--worktree-name <name>] [--leader-surface-id <id>|--surface-id <id>] [--workflow-id <id>] [--team-id <id>] [--approved <ids>|--request-approval|--approval-id <id>] [--submit] <goal> [--json]
      Apply an approved strategy. Defaults to staged workflow/team state; --request-approval creates a Feed approval without starting, --approval-id consumes an approved request, and --submit launches visible workers for supported team plans. Worktree-layer apply requires an already-open worktree workspace.
